...
class plot():
    ...
    def plot_one_series(self, x_data, y_data, label, style='', scale=False, new_subplot=False):
        ...
        if style: self.ax.plot(x_data, y_data, style, label=label)
        else: self.ax.plot(x_data, y_data, label=label)
